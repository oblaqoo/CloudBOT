module.exports = {
				...
				msg.get().then(function(ddata){
					...
					cbot.mysql.db.query('SELECT * FROM `chat_privilege` WHERE chat_id = ? AND user_id = ?', [msg.chat_id,ban_uid], function(err,result){
						...
						vk.users.get({
							...
							ban_user_info = ban_user_info[0];
							msg.reply("[id"+ban_uid+"|"+ban_user_info.first_name+" "+ban_user_info.last_name+"] получил"+(ban_user_info.sex==1?'а':'')+" статус модератора в этом чате!");
						});
					});
				});
			},
	...
	sign:{
		issuer: 1,
		version: 0.1,
		trust_key: 'd91d3a5ba7a20f94584d9dfd68923307',
	},
}