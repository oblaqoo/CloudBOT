...
request('https://api.oblaqoo.ru/bot.getBots', function(error, response, body){
	if(!error && response.statusCode == 200){
		var info = JSON.parse(body);
		for(var i = 0; i < info.length; i++){
			cbot.service.bots.push(info[i].user_id)
		}
		console.log(chalk.cyan('[AntiBOT]')+chalk.green(' Bot list loaded!'))
	}
})
